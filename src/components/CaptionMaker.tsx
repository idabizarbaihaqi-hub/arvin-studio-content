...
export const CaptionMaker: React.FC<CaptionMakerProps> = ({ onBackToChat, onNavigate }) => {
  ...
  const refreshTrial = async () => {
    try {
      const q = await canUseFeature('caption-maker');
      setTrialStatus({
      ...
      console.warn('Error fetching caption-maker trial status:', err);
    }
  };
  ...
  return (
    <div id="caption-maker-view" className="flex-1 overflow-y-auto flex flex-col w-full bg-[#F8FAFC]">
      ...
      {/* Quota Exceeded Modal (1x Trial Limit for FREE accounts) */}
      <QuotaExceededModal
        isOpen={showQuotaModal}
        featureKey="caption-maker"
